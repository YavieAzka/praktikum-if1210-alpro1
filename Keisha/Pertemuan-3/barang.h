...
void prosesTransaksi(Barang *b, int jumlah, int jenisTransaksi){
  if(jenisTransaksi == 1){ // Barang masuk
    b->stok += jumlah;
  }
  if(jenisTransaksi == 0){ // Barang keluar 
    if(b->stok - jumlah < 0){
      b->stok = 0;
      printf("Peringatan: Stok %s tidak mencukupi!", b->nama);
    }
    else{
      b->stok -= jumlah;
    }
...
float hitungTotalAset(Barang daftarBarang[], int jumlahBarang){
  ...
  for(int i = 0; i < jumlahBarang; i++){
    total_aset += (daftarBarang[i].stok * daftarBarang[i].hargaSatuan);
  }
  return total_aset;
}
